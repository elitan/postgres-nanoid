...
CREATE OR REPLACE FUNCTION nanoid(
    ...
    prefix text DEFAULT '' -- New parameter for the prefix
)
    RETURNS text 
    LANGUAGE plpgsql
    VOLATILE
...
DECLARE
    ...
    mask           int := 63;
    step           int := 34;
    finalId        text; -- Variable to hold the final ID with prefix
BEGIN
    ...
    finalId := prefix || nanoid_optimized(size, alphabet, mask, step); -- Prepend the prefix
    RETURN finalId;
END
$$;
